...
export default function CreateProjectModal(p: Props) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 60, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '12vh' }}>
      ...
      <div style={{ position: 'relative', width: 480, maxWidth: '92vw', background: 'var(--bg-surface-1)', border: '1px solid var(--border-subtle)', borderRadius: 10, boxShadow: 'var(--shadow-overlay-200)', display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '18px 20px 0 20px', fontSize: 16, fontWeight: 600, color: 'var(--txt-primary)' }}>Create project</div>
        <div style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div style={{ display: 'flex', gap: 10 }}>
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 4 }}>
              <label style={{ fontSize: 12, fontWeight: 500, color: 'var(--txt-tertiary)' }}>Name</label>
              <input value={p.name} onChange={(e) => p.onName(e.target.value)} placeholder="Project name" autoFocus style={fieldInput} {...focusProps} />
            </div>
            <div style={{ width: 110, display: 'flex', flexDirection: 'column', gap: 4 }}>
              <label style={{ fontSize: 12, fontWeight: 500, color: 'var(--txt-tertiary)' }}>ID</label>
              <input value={p.ident} onChange={(e) => p.onIdent(e.target.value)} placeholder="PROJ" style={{ ...fieldInput, fontWeight: 500, textTransform: 'uppercase' }} {...focusProps} />
            </div>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            <label style={{ fontSize: 12, fontWeight: 500, color: 'var(--txt-tertiary)' }}>Description</label>
            <textarea
              value={p.desc} onChange={(e) => p.onDesc(e.target.value)} placeholder="What is this project about?" rows={3}
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, padding: '12px 20px', borderTop: '1px solid var(--border-subtle)' }}>
          ...
          >
            {p.pending ? 'Creating...' : 'Create project'}
          </button>
        </div>
      </div>
    </div>
